...
* Full name: `\Qubus\Support\Collection\ArrayList`
...
|-----------|------------|------------------------------------------------------------------------------------------------------|
| `$type`   | **string** | The expected type of elements
...
```php
public set(int $index, mixed $element): self
```
...
```php
public removeRange(int $fromIndex, int $toIndex): void
```
...
| Parameter    | Type    | Description            |
|--------------|---------|------------------------|
| `$fromIndex` | **int** | Inclusive start index. |
| `$toIndex`   | **int** | Exclusive end index.   |
...
```php
private assertType(mixed $value): void
```

...
| Parameter | Type | Description |
|-----------|------|-------------|
| `$method` | **string** | One of 5 HTTP Methods, or a pipe-separated list<br />of multiple HTTP Methods (GET&amp;#124;POST&amp;#124;PATCH&amp;#124;PUT&amp;#124;DELETE) |
| `$subdomain` | **?string** | Domain of route. |
| `$route` | **string** | The route regex, custom regex must start with an @.<br />You can use multiple pre-set regex filters, like [i:id] |
| `$target` | **mixed** | The target where this route should point to. Can be anything. |
| `$name` | **null&#124;string** | Optional name of this route. Supply if you want to<br />reverse route this url in your application. |

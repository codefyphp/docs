...
* Full name: `\Qubus\Expressive\Schema\Compiler\SQLite`
* Parent class: [`\Qubus\Expressive\Schema\Compiler`](../Compiler.md)
...
```php
public handleModifierAutoincrement(\Qubus\Expressive\Schema\BaseColumn $column): string
```
...
```php
public handlePrimaryKey(\Qubus\Expressive\Schema\CreateTable $schema): string
```
...
```php
public currentDatabase(?string $dsn = null): array
```

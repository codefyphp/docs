...
* Full name: `\Qubus\Validation\Rules\Url`
...
```php
public forScheme(array|string $schemes): self
```
...
```php
public validateBasic(mixed $value): bool
```
...
### validateCommonScheme

Validate $value is correct $scheme format.

```php
public validateCommonScheme(mixed $value, mixed $scheme = null): bool
```
...
### validateMailtoScheme

Validate the $value is mailto scheme format.

```php
public validateMailtoScheme(mixed $value): bool
```
...
```php
public validateJdbcScheme(mixed $value): bool
```

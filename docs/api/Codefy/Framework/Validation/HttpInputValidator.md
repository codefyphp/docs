...
* Full name: `\Codefy\Framework\Validation\HttpInputValidator`
* This class implements:
  [`\Codefy\Framework\Validation\DataValidator`](./DataValidator.md)
...
```php
final private __construct(\Psr\Http\Message\ServerRequestInterface $request): mixed
```
...
```php
public static make(\Psr\Http\Message\ServerRequestInterface $request): static
```
...
```php
public setValidator(\Qubus\Validation\Validation $validator): $this
```
...
```php
public setContainer(\Qubus\Injector\ServiceContainer $container): $this
```

...
| `$context` | **mixed&#124;null** | Something which is serializable that will be logged with<br />the command execution, such as the request/session information. |
...
```php
protected log(string|\Stringable $message, \Codefy\CommandBus\Command $command): mixed
```
